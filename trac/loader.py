...
def load_components(env):
    ...
    # Try to load any eggs from the plugins directory, and also plugins
    ...
    for egg in flatten([pkg_env[name][0] for name in pkg_env]):
        ...
        for name in egg.get_entry_map('trac.plugins'):
            ...
            env.log.debug('Loading egg plugin %s from %s', name, egg.location)
            egg.activate()
            try:
    ...
    # Look for Python source files in the plugins directories, which simply get
    # imported, thereby registering them with the component manager if they
